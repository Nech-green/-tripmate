...
function App(){
  ...
  return <TripView trip={trip} aId={aId} myUid={myUid} myName={myName} ln={ln} setLn={setLn} t={L[ln]}
    allTrips={objArr(trips)} onBack={function(){setAId(null);setTrip(null);}}
    onSwitch={function(tid){setAId(tid);}} />;
}
...
function TripView(props){
  ...
  var _myCl=useState(null),myCl=_myCl[0],setMyCl=_myCl[1];
  useEffect(function(){
    if(!props.myUid||!aId) return;
    var ref=fbDB.ref("userChecklists/"+props.myUid+"/"+aId);
    ref.on("value",function(snap){
      var data=snap.val();
      if(!data){
        // First time: copy from trip defaults
        var def=defaultChecklist();
        ref.set(def);
        setMyCl(def);
      } else {
        setMyCl(data);
  ...
  // Personal checklist: stored at users/{uid}/checklists/{tripId}
  useEffect(function(){
    if(!props.myUid||!aId) return;
    var ref=fbDB.ref("users/"+props.myUid+"/checklists/"+aId);
    ref.on("value",function(snap){
      var data=snap.val();
      if(!data){
        // First time: copy default checklist from trip
        var def=trip.cl||defaultChecklist();
        fbDB.ref("users/"+props.myUid+"/checklists/"+aId).set(def);
        setMyCl(def);
      } else {
        setMyCl(data);
      }
  ...
  function updMyCl(path,val){
    if(!props.myUid||!aId) return;
    fbDB.ref("userChecklists/"+props.myUid+"/"+aId+"/"+path).set(val);
  }
  ...
  // Spread hotels: add hotels from other days if their check-in/out range covers this day
  var dayDate=day.dt;
  itin.forEach(function(otherDay){
    if(otherDay.dn===day.dn) return;
    objArr(otherDay.items||{}).forEach(function(it){
      if(!it||it.type!=="hotel"||!it.checkInDate||!it.checkOutDate) return;
      if(dayDate>=it.checkInDate&&dayDate<it.checkOutDate){
        // Check not already in dayItems
        var found=false;
        dayItems.forEach(function(di){if(di&&di.id===it.id)found=true;});
        if(!found) dayItems.push(Object.assign({},it,{_hotelSpread:true}));
      }
    });
  });
  ...
  var _dragIdx=useState(null),dragIdx=_dragIdx[0],setDragIdx=_dragIdx[1];
  function handleDragStart(idx){setDragIdx(idx);}
  function handleDragOver(e){e.preventDefault();}
  function handleDrop(targetIdx){
    if(dragIdx===null||dragIdx===targetIdx||!day.items) return;
    // Get ordered item keys
    var keys=Object.keys(day.items);
    var items=keys.map(function(k){return day.items[k];}).filter(function(x){return x&&!x._hotelSpread;});
    if(dragIdx>=items.length||targetIdx>=items.length) return;
    // Reorder
    var dragged=items.splice(dragIdx,1)[0];
    items.splice(targetIdx,0,dragged);
    // Rebuild items object with new order
    var newItems={};
    items.forEach(function(it,i){newItems[it.id]=Object.assign({},it,{order:i});});
    updF("itin/d"+sD+"/items",newItems);
    setDragIdx(null);
  }
  ...
  // Trip selector dropdown state
  var _showTripSel=useState(false),showTripSel=_showTripSel[0],setShowTripSel=_showTripSel[1];
  ...
  return <div style={{maxWidth:480,margin:"0 auto",minHeight:"100vh"}}>
    <header style={{position:"sticky",top:0,zIndex:90,background:"var(--nav)",backdropFilter:"blur(16px)",borderBottom:"1px solid var(--bd)",padding:"9px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,position:"relative"}}>
        <button onClick={function(){setShowTripSel(!showTripSel);}} style={{background:"var(--sf)",border:"1px solid var(--bd)",borderRadius:8,padding:"4px 10px",cursor:"pointer",fontSize:12,color:"var(--txt)",display:"flex",alignItems:"center",gap:4}}>
          <span style={{fontSize:14}}>✈️</span>
          <span style={{fontSize:13,fontWeight:700,maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ln==="he"?trip.name:trip.nameEn}</span>
          <span style={{fontSize:10}}>▼</span>
        </button>
        {showTripSel && <div style={{position:"absolute",top:36,left:0,zIndex:100,background:"var(--card)",border:"1px solid var(--bd)",borderRadius:12,boxShadow:"0 8px 24px rgba(0,0,0,.15)",minWidth:200,overflow:"hidden"}}>
          {(props.allTrips||[]).map(function(tr){return <button key={tr.id} onClick={function(){props.onSwitch(tr.id);setShowTripSel(false);}} style={{width:"100%",padding:"10px 14px",background:tr.id===aId?"var(--acS)":"transparent",border:"none",borderBottom:"1px solid var(--bd)",cursor:"pointer",textAlign:"inherit",display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:12}}>✈️</span>
            <span style={{fontSize:12,fontWeight:tr.id===aId?700:500,color:"var(--txt)"}}>{ln==="he"?tr.name:tr.nameEn||tr.name}</span>
          </button>;})}
          <button onClick={function(){props.onBack();setShowTripSel(false);}} style={{width:"100%",padding:"10px 14px",background:"transparent",border:"none",cursor:"pointer",textAlign:"inherit",color:"var(--ac)",fontWeight:600,fontSize:12}}>📋 {ln==="he"?"כל הטיולים":"All Trips"}</button>
        </div>}
      </div>
    ...
    <main style={{padding:14,paddingBottom:78}}>
      ...
      {tab==="home" && <div>
        ...
        {/* Wishlist on home */}
        {wl.length>0 && <div style={{marginTop:14}}>
          <h3 style={{fontSize:14,fontWeight:700,color:"var(--txt)",margin:"0 0 8px"}}>❤️ {t.wish} ({wl.length})</h3>
          {wl.slice(0,5).map(function(w){if(!w)return null;return <div key={w.id} style={{background:"var(--card)",border:"1px solid var(--bd)",borderRadius:10,padding:10,marginBottom:5,display:"flex",alignItems:"center",gap:8,borderRight:"3px solid #E84393"}}>
            <span style={{fontSize:16}}>❤️</span>
            <div style={{flex:1}}><p style={{fontSize:12,fontWeight:600,color:"var(--txt)",margin:0}}>{ln==="he"?w.title:w.titleEn||w.title}</p><p style={{fontSize:10,color:"var(--mt)",margin:0}}>📍 {w.location||""}</p></div>
            <button onClick={function(){setMvW(mvW===w.id?null:w.id);setTab("wishlist");}} style={{background:"var(--acS)",border:"none",borderRadius:5,padding:"3px 7px",cursor:"pointer",color:"var(--ac)",fontSize:9,fontWeight:600}}>{t.move}</button>
          </div>;})}
        </div>}
      </div>}
      ...
      {tab==="itinerary" && <div>
        ...
        {dayItems.filter(function(x){return x;}).sort(function(a,b){return (a.order||0)-(b.order||0);}).map(function(it,idx){if(!it)return null;var emoji=it.transportSub?EMJ[it.transportSub]:(EMJ[it.type]||"📌");var typeName=it.type==="transport"&&it.transportSub?(t.trSub[it.transportSub]||""):(t.ty[it.type]||"");var isSpread=it._hotelSpread;
          return <div key={(it.id||"")+idx} draggable={!isSpread} onDragStart={function(){handleDragStart(idx);}} onDragOver={handleDragOver} onDrop={function(){handleDrop(idx);}}
            style={{background:dragIdx===idx?"var(--acS)":"var(--card)",border:"1px solid "+(dragIdx===idx?"var(--ac)":"var(--bd)"),borderRadius:11,padding:11,marginBottom:8,borderTop:"3px solid "+(CLR[it.type]||"#888"),cursor:"grab",opacity:isSpread?0.7:1,position:"relative"}}>
            {/* Drag handle */}
            {!isSpread && <div style={{position:"absolute",top:12,left:isR?"auto":6,right:isR?6:"auto",color:"var(--mt)",fontSize:14,cursor:"grab",touchAction:"none"}}>⠿</div>}
            <div onClick={function(){if(!isSpread){setEIt(it);setMod("editI");}}} style={{paddingRight:isR?20:0,paddingLeft:isR?0:20}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                <span style={{fontSize:10,fontWeight:600,color:CLR[it.type]||"#888"}}>{emoji} {typeName} · {it.time||""}{it.endTime?" — "+it.endTime:""}</span>
                <div style={{display:"flex",gap:4,alignItems:"center"}}>
                  {isSpread && <span style={{fontSize:8,color:"#A855F7",background:"#A855F712",padding:"1px 5px",borderRadius:4}}>🏨 {ln==="he"?"לינה":"Stay"}</span>}
                  {it.price>0 && <span style={{fontSize:10,fontWeight:700,color:"var(--ok)",background:"var(--ok)"+"15",padding:"1px 6px",borderRadius:5}}>{money(it.price,it.priceCur||"ILS")}</span>}
                  {!isSpread && <span style={{color:"var(--mt)",fontSize:11}}>✏️</span>}
                </div>
              </div>
      ...
      {tab==="checklist" && <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <h2 style={{fontSize:16,fontWeight:700,color:"var(--txt)",margin:0}}>✅ {ln==="he"?"הצ'קליסט שלי":"My Checklist"}</h2>
          <Bt ch={"+ "+t.addItem} fn={function(){setNci({text:"",textEn:"",cat:"misc",qty:1});setMod("addCl");}} st={{padding:"5px 10px",fontSize:10}} />
        </div>
        <p style={{fontSize:10,color:"var(--mt)",marginBottom:10}}>{ln==="he"?"רשימה אישית — רק אתה רואה אותה":"Personal list — only you can see it"}</p>

        {clCat==="all" && CL_CATS.map(function(cat){
          var catItems=cl.filter(function(c){return c&&c.cat===cat;});
          var checked=catItems.filter(function(c){return c.done;}).length;
          ...
          return <button key={cat} onClick={function(){setClCat(cat);}} style={{width:"100%",background:"var(--card)",border:"1px solid var(--bd)",borderRadius:12,padding:14,marginBottom:8,cursor:"pointer",textAlign:"inherit",display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{width:36,height:36,borderRadius:"50%",background:checked===total&&total>0?"var(--ok)":"var(--sf)",border:"2px solid "+(checked===total&&total>0?"var(--ok)":"var(--bd)"),display:"flex",alignItems:"center",justifyContent:"center"}}>
              {checked===total&&total>0 ? <span style={{color:"#fff",fontSize:16}}>✓</span> : <span style={{fontSize:10,fontWeight:700,color:"var(--ac)"}}>{total>0?Math.round(checked/total*100):0}%</span>}
            </div>
          </button>;
        })}
        ...
        {clCat!=="all" && <div>
          ...
          {cl.filter(function(c){return c&&c.cat===clCat;}).map(function(c){if(!c)return null;var ck=c.done;return <div key={c.id} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 10px",background:"var(--card)",border:"1px solid var(--bd)",borderRadius:8,marginBottom:5}}>
            <button onClick={function(){updMyCl(c.id+"/done",!ck);}} style={{width:22,height:22,borderRadius:6,border:"2px solid "+(ck?"var(--ok)":"var(--bd)"),background:ck?"var(--ok)":"transparent",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,flexShrink:0,cursor:"pointer"}}>{ck?"✓":""}</button>
            <div style={{flex:1}}>
              ...
              <span style={{display:"inline-flex",alignItems:"center",gap:2,marginRight:4,marginLeft:4}}>
                ...
                <input type="number" value={c.qty||1} onChange={function(e){var nq=parseInt(e.target.value)||1;updMyCl(c.id+"/qty",nq);}} style={{width:28,padding:"1px 2px",borderRadius:4,border:"1px solid var(--bd)",background:"var(--sf)",color:"var(--txt)",fontSize:10,textAlign:"center",boxSizing:"border-box"}} onClick={function(e){e.stopPropagation();}} />
              </span>
            </div>
            <button onClick={function(){updMyCl(c.id,null);}} style={{background:"none",border:"none",color:"var(--mt)",cursor:"pointer",opacity:.3,fontSize:11}}>🗑</button>
          </div>;})}
        </div>}
        ...
        <Mod open={mod==="addCl"} onClose={function(){setMod(null);}} title={t.addItem} ch={<div>
          ...
          <Bt ch={t.save} fn={function(){if(!nci.text&&!nci.textEn)return;var cid=xid();updMyCl(cid,{id:cid,text:nci.text,textEn:nci.textEn,cat:nci.cat,qty:nci.qty,done:false});setMod(null);}} st={{width:"100%",justifyContent:"center",marginTop:6}} />
        </div>} />
      </div>}
