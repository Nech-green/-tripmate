...
function App(){
  ...
  var _named=useState(false),named=_named[0],setNamed=_named[1];
  ...
  useEffect(function(){
    fbSignIn(function(user){
      setMyUid(user.uid);
      fbGetUserName(user.uid, function(n){
        if(n){setMyName(n);setNamed(true);}
        setLoading(false);
      });
    });
  },[]);
  ...
  if(!named) return <div style={{maxWidth:400,margin:"0 auto",padding:"60px 20px",textAlign:"center"}}>
    <h1 style={{fontSize:28,fontWeight:800,color:"var(--txt)",marginBottom:8}}>✈️ TripMate</h1>
    <p style={{color:"var(--mt)",marginBottom:24}}>{t.yourName}</p>
    <Inp value={myName} onChange={function(e){setMyName(e.target.value);}} style={{textAlign:"center",fontSize:16,marginBottom:16}} />
    <Bt ch={t.setName} fn={function(){if(myName.trim()){fbSetUserName(myUid,myName.trim());setMyName(myName.trim());setNamed(true);}}} st={{width:"100%",justifyContent:"center",padding:"12px 20px",fontSize:15}} />
    <button onClick={function(){setLn(ln==="he"?"en":"he");}} style={{background:"none",border:"none",color:"var(--mt)",cursor:"pointer",marginTop:16,fontSize:12}}>{ln==="he"?"English":"עברית"}</button>
  </div>;
  ...
  return <TripView trip={trip} aId={aId} myUid={myUid} ln={ln} setLn={setLn} t={L[ln]} onBack={function(){setAId(null);setTrip(null);}} />;
}
  ...
  return <div style={{maxWidth:480,margin:"0 auto",padding:"28px 16px",minHeight:"100vh"}}>
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:20}}>
      ...
      <div style={{display:"flex",gap:6,alignItems:"center"}}>
        <span style={{fontSize:11,color:"var(--mt)"}}>{props.myName}</span>
    ...
    {props.trips.map(function(tr){return <button key={tr.id} onClick={function(){props.onPick(tr.id);}} style={{width:"100%",background:"var(--card)",border:"1px solid var(--bd)",borderRadius:14,padding:14,marginBottom:8,cursor:"pointer",textAlign:"inherit"}}><h3 style={{fontSize:15,fontWeight:700,color:"var(--txt)",margin:0}}>{ln==="he"?tr.name:tr.nameEn||tr.name}</h3><p style={{fontSize:11,color:"var(--mt)",margin:"2px 0 0"}}>{t.invite}: {tr.inv}</p></button>;})}
  ...
  return <div style={{maxWidth:480,margin:"0 auto",minHeight:"100vh"}}>
    ...
    <main style={{padding:14,paddingBottom:78}}>
      ...
      {tab==="checklist" && <div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:8}}><span style={{fontSize:10,color:"var(--mt)",fontWeight:600}}>{t.view}</span>{mems.map(function(m){return <button key={m.id} onClick={function(){setVA(m.id);}} style={{width:22,height:22,borderRadius:"50%",border:vA===m.id?"2px solid "+m.c:"2px solid transparent",background:m.c,color:"#fff",fontSize:8,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>{m.av}</button>;})}</div>
        <div style={{display:"flex",gap:4,overflowX:"auto",marginBottom:10}}><Chip on={clCat==="all"} c="var(--ac)" fn={function(){setClCat("all");}} ch={<span>{t.all}</span>} />{CL_CATS.map(function(cat){return <Chip key={cat} on={clCat===cat} c="var(--ac)" fn={function(){setClCat(cat);}} ch={<span>{CL_CAT_E[cat]} {ln==="he"?CL_CAT_HE[cat]:CL_CAT_EN[cat]}</span>} />;})}</div>
        {cl.filter(function(c){return c&&(clCat==="all"||c.cat===clCat);}).map(function(c){if(!c)return null;var ck=c.ck&&c.ck[vA];return <div key={c.id} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 10px",background:"var(--card)",border:"1px solid var(--bd)",borderRadius:8,marginBottom:5}}><button onClick={function(){updF("cl/"+c.id+"/ck/"+vA,!ck);}} style={{width:22,height:22,borderRadius:6,border:"2px solid "+(ck?"var(--ok)":"var(--bd)"),background:ck?"var(--ok)":"transparent",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,flexShrink:0,cursor:"pointer"}}>{ck?"✓":""}</button><div style={{flex:1}}><span style={{fontSize:13,color:ck?"var(--mt)":"var(--txt)",textDecoration:ck?"line-through":"none"}}>{ln==="he"?c.text:c.textEn||c.text}</span>{c.qty>1&&<span style={{fontSize:10,color:"var(--warn)",marginRight:6,marginLeft:6}}>×{c.qty}</span>}<span style={{fontSize:9,color:"var(--mt)"}}>{CL_CAT_E[c.cat]||"📦"}</span></div><button onClick={function(){updF("cl/"+c.id,null);}} style={{background:"none",border:"none",color:"var(--mt)",cursor:"pointer",opacity:.3,fontSize:11}}>🗑</button></div>;})}
